...
export class FilterService {
  objectForIterration: any = []; /// ARRAY HELPER FOR ITERRATION ////
  titles: any = [];
  previousFilter: string;
  private checkedObjects = []; /// ARRAY FOR CHECKED CHECKBOXES ////
    ...
    if (checkedValue.isChecked) {
      ...
      this.objectForIterration.forEach(el => {
        ...
        if (!el[key].length) {
            ...
            this.objectForIterration = this.objectForIterration.filter(
              el => !el[key]
            );
